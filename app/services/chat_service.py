...
# =============================================================================
# SQL Fixer - Fix common LLM-generated SQL issues
# =============================================================================
...
def fix_sql_syntax(sql: str) -> str:
    """
    Fix common SQL syntax issues generated by LLM.
    
    Fixes:
    1. UNION ALL with ORDER BY/LIMIT without parentheses
    2. Multiple statements (only keep first SELECT)
    """
    if not sql:
        return sql
    ...
    # Check if SQL contains UNION with ORDER BY (common LLM mistake)
    if 'UNION' in sql.upper() and ('ORDER BY' in sql.upper() or 'LIMIT' in sql.upper()):
        logger.warning("Detected UNION with ORDER BY/LIMIT - simplifying query")
        
        # Strategy: Keep only the first SELECT before UNION
        # This is safer than trying to fix parentheses
        union_pattern = re.compile(r'\s+UNION\s+(ALL\s+)?SELECT', re.IGNORECASE)
        match = union_pattern.search(sql)
        
        if match:
            # Get the first part before UNION
            first_select = sql[:match.start()].strip()
            
            # Remove LIMIT 1 if present (we want all data for comparison)
            first_select = re.sub(r'\s+LIMIT\s+\d+\s*$', '', first_select, flags=re.IGNORECASE)
            
            logger.info(f"Simplified SQL from UNION to single SELECT")
            return first_select
...
class ChatService:
    ...
    async def ask(
        ...
        # =====================================================================
        # Step 1.5: Fix common SQL syntax issues
        # =====================================================================
        ...
        try:
            ...
            await self._save_to_metadata(question, response, filters or {}, report_id)
            if not include_sql:
                response.sql = None
            return response
        ...
        # Save to metadata (non-blocking, won't fail request)
        await self._save_to_metadata(question, response, filters or {}, report_id)
        
        # Remove SQL from response if not requested
        if not include_sql:
            response.sql = None
    ...
    async def _save_to_metadata(
        ...
        """
        Save chat interaction to metadata.llm_metadata table.
        ...
        try:
            ...
            async with AnalyticsSessionLocal() as session:
                await session.execute(
                    text('''
                        INSERT INTO metadata.llm_metadata (
                            user_question, chatbot_answer, response_time_ms,
                            tokens_used, sql_generated, status, error_message,
                            row_count, estimated_cost_usd, report_id, filters_applied
                        ) VALUES (
                            :question, :answer, :response_time,
                            :tokens, :sql, :status, :error,
                            :row_count, :cost, :report_id, CAST(:filters AS jsonb)
                        )
                    '''),
                    {
                        "question": question,
                ...
                logger.debug("Chat saved to metadata.llm_metadata")
